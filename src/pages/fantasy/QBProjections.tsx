...
const SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/1YPjEIh4Ih5yc73zzQcEnuXXe_DWHOr8wTOfckrm-gO0/gviz/tq?tqx=out:csv';
...
export default function QBProjections() {
  const [headers, setHeaders] = useState<string[]>([]);
  const [rows, setRows] = useState<string[][]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortColumn, setSortColumn] = useState<number | null>(null);
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('desc'); // Default to descending for projections (higher is better)
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const headerRowIdx = data.findIndex(
          row => row.includes('Player') && row.includes('Passing Yards Line')
        );
        if (headerRowIdx === -1) {
  ...
  const handleSort = (colIdx: number) => {
    if (sortColumn === colIdx) {
      ...
      setSortDirection('desc'); // Default to descending for projections (higher is better)
    }
  };

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-4">QB Projections</h1>
      {loading ? (
                  ...
                  {row.map((cell, j) => (
                    <td
                      ...
                      className={
                        ...
                        (j === 0 ? ' font-bold bg-white sticky left-0 z-10 shadow-lg' : '')
                      }
                    >
                      {cell}
                    </td>

...
export async function persistReceptionProjections(
  ...
  const rows = results
    ...
    .map((r) => ({
      ...
      opp_td_rate_allowed: r.opponentTdRateAllowed,
      opp_catch_pct_allowed: r.opponentCatchPctAllowed,
      targets_per_game: r.targetsPerGame,
      catch_pct_season: r.catchPctSeason,
      reception_debt: r.receptionDebt,
      sharp_score: r.sharpScore,

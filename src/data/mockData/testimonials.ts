import { Testimonial } from '../../types/testimonials';

export const testimonials: Testimonial[] = [
  {
    name: "Mike R.",
    image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The course fit model helped me identify Viktor Hovland as the perfect play for the Memorial Tournament. The putting adjustment for bentgrass greens was crucial.",
    winAmount: "$15,000",
    platform: "FanDuel Sportsbook",
    tournamentWins: [
      {
        tournament: "Memorial Tournament",
        player: "Viktor Hovland",
        odds: "+2800",
        year: 2023
      }
    ]
  },
  {
    name: "Sarah L.",
    image: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The wind model for The Open Championship was spot on. Caught Brian Harman at +8000 before his historic win at Royal Liverpool.",
    winAmount: "$40,000",
    platform: "DraftKings Sportsbook",
    tournamentWins: [
      {
        tournament: "The Open Championship",
        player: "Brian Harman",
        odds: "+8000",
        year: 2023
      }
    ]
  },
  {
    name: "David K.",
    image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The proximity stats correlation with Augusta National helped me identify Brooks Koepka as a strong play for the Masters. The historical performance analyzer was crucial.",
    winAmount: "$25,000",
    platform: "BetMGM",
    tournamentWins: [
      {
        tournament: "Masters Tournament",
        player: "Brooks Koepka",
        odds: "+2500",
        year: 2023
      }
    ]
  },
  {
    name: "Ryan M.",
    image: "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The lineup optimizer's correlation feature is unreal. Won the DraftKings Millionaire Maker during The Masters by stacking players with similar Augusta success patterns.",
    winAmount: "$1,000,000",
    platform: "DraftKings",
    fantasySuccess: {
      tournamentName: "Masters Millionaire Maker",
      entryFee: "$20",
      fieldSize: "235,000 entries",
      lineup: ["Jon Rahm", "Brooks Koepka", "Phil Mickelson", "Jordan Spieth", "Cameron Young", "Sahith Theegala"]
    }
  },
  {
    name: "Jennifer K.",
    image: "https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The ownership projections and leverage calculator helped me find the perfect contrarian plays for the PGA Championship GPP.",
    winAmount: "$500,000",
    platform: "FanDuel",
    fantasySuccess: {
      tournamentName: "PGA Championship Monster",
      entryFee: "$15",
      fieldSize: "180,000 entries",
      lineup: ["Brooks Koepka", "Viktor Hovland", "Tyrrell Hatton", "Eric Cole", "Taylor Moore", "Michael Block"]
    }
  },
  {
    name: "Chris P.",
    image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The course history correlation tool helped me identify Wyndham Clark as a strong value play for the US Open at LA Country Club.",
    winAmount: "$35,000",
    platform: "Caesars Sportsbook",
    tournamentWins: [
      {
        tournament: "US Open",
        player: "Wyndham Clark",
        odds: "+7500",
        year: 2023
      }
    ]
  },
  {
    name: "Tom B.",
    image: "https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The weather impact tool helped me build the perfect lineup for The Open Championship. Stacked early/late wave players with favorable conditions.",
    winAmount: "$250,000",
    platform: "DraftKings",
    fantasySuccess: {
      tournamentName: "The Open Championship Special",
      entryFee: "$25",
      fieldSize: "150,000 entries",
      lineup: ["Brian Harman", "Tommy Fleetwood", "Jason Day", "Min Woo Lee", "Adrian Meronk", "Richie Ramsay"]
    }
  },
  {
    name: "Kevin P.",
    image: "https://images.unsplash.com/photo-1463453091185-61582044d556?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The course history correlation tool helped me identify the perfect game stack for TPC Sawgrass. Paired players with strong Par 5 scoring.",
    winAmount: "$300,000",
    platform: "FanDuel",
    fantasySuccess: {
      tournamentName: "Players Championship Supreme",
      entryFee: "$50",
      fieldSize: "95,000 entries",
      lineup: ["Scottie Scheffler", "Max Homa", "Justin Thomas", "Corey Conners", "Keith Mitchell", "Taylor Montgomery"]
    }
  },
  {
    name: "Michelle S.",
    image: "https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The lineup builder's advanced stats correlation helped me identify the perfect low-owned stack for the U.S. Open GPP.",
    winAmount: "$450,000",
    platform: "DraftKings",
    fantasySuccess: {
      tournamentName: "U.S. Open Mega GPP",
      entryFee: "$100",
      fieldSize: "75,000 entries",
      lineup: ["Wyndham Clark", "Rory McIlroy", "Rickie Fowler", "Min Woo Lee", "Harris English", "Dustin Johnson"]
    }
  },
  {
    name: "Alex T.",
    image: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&q=80&w=100&h=100",
    quote: "The putting surface analysis helped me identify Jon Rahm as a strong play for the Masters. The bentgrass performance metrics were key.",
    winAmount: "$50,000",
    platform: "PointsBet",
    tournamentWins: [
      {
        tournament: "Masters Tournament",
        player: "Jon Rahm",
        odds: "+1000",
        year: 2023
      }
    ]
  }
];